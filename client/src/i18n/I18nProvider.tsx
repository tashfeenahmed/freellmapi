...
// `navigator.language` returns values like `zh`, `zh-CN`, `zh-Hans`, `en-US`.
// We snap to the closest supported locale so the first-visit detection is
// forgiving (e.g. a `zh-Hans` browser still gets our `zh-CN` strings).
...
export function I18nProvider({ children, initialLocale }: I18nProviderProps) {
  ...
  const toggleLocale = useCallback(() => {
    setLocaleState((cur) => (cur === 'en' ? 'zh-CN' : 'en'))
  }, [])
