...
function isRetryableError(err: any): boolean {
  ...
  return msg.includes('429') || msg.includes('rate limit') || msg.includes('too many requests')
    ...
    // A stale/decommissioned upstream model id (free-tier models churn often)
    // returns 404 model_not_found. For a FALLBACK proxy the correct behaviour
    // is to skip that route and fall through to the next model in the chain —
    // NOT to 502 the whole request. Pinned-model requests are pre-validated
    // against the enabled catalog (see ~line 266) so a genuine typo still 400s
    // before this loop; only catalog rows whose upstream id went stale reach here.
    || msg.includes('404') || msg.includes('not found') || msg.includes('does not exist')
    || msg.includes('model_not_found') || msg.includes('unavailable_model')
    // A per-key account restriction (e.g. Groq "Organization has been restricted")
    // is a permanent failure for THAT key but the other keys / models are fine.
    // Treat it as retryable so routeRequest skips the bad key and falls through
    // instead of 502-ing a request that other keys could have served.
