...
export { isRetryableError, isPaymentRequiredError, isModelNotFoundError, isModelAccessForbiddenError, isProviderAuthFailoverError };
  ...
  const messages: ChatMessage[] = parsed.data.messages.map((m): ChatMessage => {
    if (m.role === 'assistant') {
      ...
      const assistantContent = (hasToolCalls
        ? (m.content ?? null)
        : (isEmptyContent ? '' : m.content!)) as ChatMessage['content'];
      return {
        role: 'assistant',
