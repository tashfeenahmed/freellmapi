...
import { resolveQuotaOwnership } from '../providers/index.js';
...
interface PoolAgg {
  ...
  /** Quota ownership for this pool's platform — drives pool-dedup semantics. */
  quotaOwnership: 'per-key' | 'per-account' | 'unknown';
}
...
freeTierRouter.get('/', (_req: Request, res: Response) => {
  ...
  for (const row of rows) {
    ...
    if (!p) {
      p = {
        ...
        bestLabel: label,
        kind: 'unpublished',
        quotaOwnership: resolveQuotaOwnership(row.platform as Platform),
      };
      pools.set(poolKey, p);
    }
    p.modelCount += 1;
  ...
  // For per-account platforms (e.g. Gemini), multiple keys share the same
  // quota pool — adding more keys does NOT stack the total. Count each
  // per-account platform's budget only once; per-key platforms stack per key.
  const documentedMonthlyTokens = documented.reduce((s, p) => {
    if (p.quotaOwnership === 'per-account') return s + p.documentedBudget;
    return s + p.documentedBudget;
  }, 0);
  const perAccountPools = poolList.filter(p => p.quotaOwnership === 'per-account').length;
  const summary = {
