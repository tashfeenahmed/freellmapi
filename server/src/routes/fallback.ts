...
// ── OpenRouter capabilities sync ──────────────────────────────────────────
// POST /sync-capabilities — pull live model metadata from OpenRouter's
// /api/frontend/models and apply context_window, vision, reasoning, and
// rate-limit data to every enabled model in the catalog.
fallbackRouter.post('/sync-capabilities', (_req: Request, res: Response) => {
  try {
    const result = syncCapabilitiesDetailed(getDb());
    res.json(result);
  } catch (err) {
