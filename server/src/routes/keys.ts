...
keysRouter.post('/custom', (req: Request, res: Response) => {
  ...
  const upsert = db.transaction(() => {
    ...
    // Append to all profiles if not already present
    const profiles = db.prepare('SELECT id FROM profiles').all() as { id: number }[];
    const checkProfile = db.prepare('SELECT 1 FROM profile_models WHERE profile_id = ? AND model_db_id = ?');
    const getMaxProfile = db.prepare('SELECT COALESCE(MAX(priority), 0) AS m FROM profile_models WHERE profile_id = ?');
    const insertProfile = db.prepare('INSERT INTO profile_models (profile_id, model_db_id, priority, enabled) VALUES (?, ?, ?, 1)');
    
    for (const p of profiles) {
      if (!checkProfile.get(p.id, modelRow.id)) {
        const pMax = getMaxProfile.get(p.id) as { m: number };
        insertProfile.run(p.id, modelRow.id, pMax.m + 1);
      }
    }
