...
describe('response cache', () => {
  ...
  describe('computeCacheKey', () => {
    ...
    // #892: explicit-but-default-valued sampling params must NOT change the
    // key — a client that always serializes top_p:1 / presence_penalty:0 /
    // frequency_penalty:0 must hit the same entry as one that omits them.
    it('ignores explicit default-valued sampling params (#892)', () => {
      const messages = [msg('user', 'hello')];
    ...
    it('still keys non-default sampling param values (#892)', () => {
      const messages = [msg('user', 'hello')];
