...
function migrateModelsV4(db: Database.Database) {
  ...
  db.prepare(`UPDATE fallback_config SET priority = 6 WHERE model_db_id = (SELECT id FROM models WHERE platform = 'cerebras' AND model_id = 'qwen-3-235b-a22b-instruct-2507')`).run();
...
/**
 * V12 (2026-06-04): the chain returned "no working model" for every auto-routed
 * request. Root cause confirmed by direct upstream probes (proxy bypassed):
 *  - cerebras `qwen-3-235b-a22b-instruct-2507` (introduced by V11's rename) is a
 *    DEAD id — Cerebras /v1/chat/completions returns HTTP 404 model_not_found
 *    ("does not exist or you do not have access"). Cerebras' live free catalog is
 *    only `zai-glm-4.7` and `gpt-oss-120b`. This was the first key-having route
 *    in the chain, and the proxy treated 404 as non-retryable, so the whole
 *    fallback chain short-circuited to a 502 before reaching the working
 *    google/groq routes below it. (The 404-as-retryable fix lives in proxy.ts.)
 *
 * Fix: disable the dead cerebras id, and pin the verified-working
 * groq/llama-3.3-70b-versatile (direct probe: HTTP 200 + real content; 3 healthy
 * keys; 1000 RPD) to the top of the fallback chain. Idempotent / re-runnable.
 */
function migrateModelsV12(db: Database.Database) {
  // 1) Disable the dead Cerebras model id (404 model_not_found on the live API).
  db.prepare(`
    UPDATE models SET enabled = 0
     WHERE platform = 'cerebras' AND model_id = 'qwen-3-235b-a22b-instruct-2507'
  `).run();

  // 2) Pin a verified-working free route to the top of the chain.
  //    priority 0 sorts ahead of every existing row (previous min was 1).
  db.prepare(`
    UPDATE fallback_config SET priority = 0, enabled = 1
     WHERE model_db_id = (
