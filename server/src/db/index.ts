...
// Default system prompt — injected at proxy level so every model sees the same base persona
const DEFAULT_SYSTEM_PROMPT_KEY = 'default_system_prompt';

export function getDefaultSystemPrompt(): string | undefined {
  return getSetting(DEFAULT_SYSTEM_PROMPT_KEY);
}

export function setDefaultSystemPrompt(prompt: string | undefined): void {
  if (prompt === undefined || prompt.trim() === '') {
    // Delete the setting to revert to no default
    const db = getDb();
    db.prepare('DELETE FROM settings WHERE key = ?').run(DEFAULT_SYSTEM_PROMPT_KEY);
  } else {
    setSetting(DEFAULT_SYSTEM_PROMPT_KEY, prompt.trim());
  }
}
