...
/**
 * V23 (June 2026): Add per-model cost tracking columns.
 * input_cost_per_1m  — price per 1M input tokens ($)
 * output_cost_per_1m — price per 1M output tokens ($)
 * cost_updated_at    — ISO timestamp of last manual update
 */
function migrateModelsV23Cost(db: Database.Database) {
  const columns = db.prepare('PRAGMA table_info(models)').all() as { name: string }[];
  if (!columns.some(col => col.name === 'input_cost_per_1m')) {
    db.prepare('ALTER TABLE models ADD COLUMN input_cost_per_1m REAL').run();
  }
  if (!columns.some(col => col.name === 'output_cost_per_1m')) {
    db.prepare('ALTER TABLE models ADD COLUMN output_cost_per_1m REAL').run();
  }
  if (!columns.some(col => col.name === 'cost_updated_at')) {
    db.prepare('ALTER TABLE models ADD COLUMN cost_updated_at TEXT').run();
  }
}
