...
//
// POST /custom and the scheduled custom-model sync (#674/#663/#656) register
// models against a user's own endpoint through the SAME code path, so the two
// can never drift apart. The transaction body below used to live inline in
// routes/keys.ts; it moved here verbatim so the scheduler cannot invent a
// subtly different notion of "registering a custom model".
